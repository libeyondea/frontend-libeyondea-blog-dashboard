...
const SigninCompoment = () => {
			...
			httpRequest
				...
				.then((response) => {
					if (response.data.success) {
						setCookie('accessToken', response.data.data.tokens.accessToken.token);
						setCookie('refreshToken', response.data.data.tokens.refreshToken.token);
						history.push('/');
					}
				})
				.catch((error) => {
					console.log(error);
					setErrors({ userName: error?.response?.data?.error?.message, password: error?.response?.data?.error?.message });
				})
				.finally(() => {
					setSubmitting(false);
				});
	...
	return (
		<CardComponent className="flex flex-col w-full max-w-md px-4 py-8 sm:px-6 md:px-8 lg:px-10">
